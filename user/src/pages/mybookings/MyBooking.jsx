import "./myBookings.css";
import Navbar from "../../components/navbar/Navbar";
import Footer from "../../components/footer/Footer";
import { useContext, useEffect, useState } from "react";
import { AuthContext } from "../../context/AuthContext";
import axios from "axios";
import jsPDF from "jspdf";

const MyBookings = () => {
  const { user } = useContext(AuthContext);
  const [bookings, setBookings] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const res = await axios.get(`/bookings/user/${user._id}`);
        console.log("Bookings data:", res.data);
        setBookings(res.data);
      } catch (err) {
        console.error("Failed to fetch bookings", err);
      }
      setLoading(false);
    };

    if (user) fetchBookings();
  }, [user]);

  const generatePDF = (booking) => {
    const pdf = new jsPDF('p', 'mm', 'a4');
    
    // Add title
    pdf.setFontSize(20);
    pdf.setTextColor(40, 53, 147);
    pdf.text('Booking Receipt', 105, 20, { align: 'center' });
    
    // Add booking details
    pdf.setFontSize(12);
    pdf.setTextColor(0, 0, 0);
    
    let yPosition = 40;
    
    const addField = (label, value) => {
      pdf.text(`${label}: ${value}`, 20, yPosition);
      yPosition += 10;
    };
    
    addField('Booking ID', booking._id);
    addField('User', user.username);
    addField('Lounge', booking.loungeName || booking.loungeId);
    addField('Occasion', booking.occasion);
    addField('Date', `${new Date(booking.startDate).toLocaleDateString()} - ${new Date(booking.endDate).toLocaleDateString()}`);
    // addField('Time Slot', booking.timeSlot);
    
    // Services
    pdf.text('Services:', 20, yPosition);
    yPosition += 7;
    booking.services.forEach(service => {
      pdf.text(`- ${service}`, 25, yPosition);
      yPosition += 7;
    });
    
    // Additional options
    yPosition += 5;
    addField('Generator Backup', booking.generatorBackup ? 'Yes' : 'No');
    addField('Additional Furniture', booking.additionalFurniture || 'None');
    addField('Additional Lighting', booking.additionalLighting || 'None');
    addField('Additional Waiters', booking.additionalWaiters || 'None');
    addField('Catering', booking.catering || 'None');
    
    // Total price
    yPosition += 10;
    pdf.setFontSize(14);
    pdf.setTextColor(40, 53, 147);
    pdf.text(`Total Price: $${booking.totalPrice}`, 20, yPosition);
    
    // Status
    yPosition += 10;
    pdf.text(`Status: ${booking.status}`, 20, yPosition);
    
    // Footer
    yPosition += 20;
    pdf.setFontSize(10);
    pdf.setTextColor(100, 100, 100);
    pdf.text('Thank you for your booking!', 105, yPosition, { align: 'center' });
    
    pdf.save(`Receipt_${booking._id}.pdf`);
  };

  return (
    <div>
      <Navbar />
      <div className="myBookingsContainer">
        <h1>My Bookings</h1>
        {loading ? (
          <p>Loading your bookings...</p>
        ) : bookings.length === 0 ? (
          <p>No bookings found.</p>
        ) : (
          <div className="myBookingsList">
            {bookings.map((booking) => (
              <div className="myBookingCard" key={booking._id}
                style={{
                  ...(booking.status === "Rejected" && { backgroundColor: "#ffcccc" }),
                  ...(booking.status === "Booking Done" && { backgroundColor: "#4caf50" }),
                }}
              >
                <div className="statusBadge"
                  style={booking.status === "Rejected" ? { backgroundColor: "Red" } : {}}
                >
                  <p><strong>Status:</strong> {booking.status}</p>
                  {booking.status === "Payment pending" && (
                    <div className="paymentPendingWrapper">
                      <img
                        className="paymentPendingImage"
                        src="data:image/png;base64,"
                        alt="Payment Pending"
                      />
                    </div>
                  )}
                  {booking.status === "Booking Done" && (
                  <button 
                    className="downloadBtn" 
                    onClick={() => generatePDF(booking)}
                  >
                    Download Receipt
                  </button>
                )}
                </div>

                <h3>{booking.occasion}</h3>
                <p><strong>Lounge Name:</strong> {booking.loungeName || booking.loungeId}</p>
                <p><strong>Date:</strong> {new Date(booking.startDate).toLocaleDateString()} - {new Date(booking.endDate).toLocaleDateString()}</p>
                {/* <p><strong>Time Slot:</strong> {booking.timeSlot}</p> */}
                <p><strong>Services:</strong> {booking.services.join(", ")}</p>
                <p><strong>Generator Backup:</strong> {booking.generatorBackup ? "Yes" : "No"}</p>
                <p><strong>Furniture:</strong> {booking.additionalFurniture || "None"}</p>
                <p><strong>Lighting:</strong> {booking.additionalLighting || "None"}</p>
                <p><strong>Waiters:</strong> {booking.additionalWaiters || "None"}</p>
                <p><strong>Catering:</strong> {booking.catering || "None"}</p>
                <p><strong>Total Price:</strong> ${booking.totalPrice}</p>

              </div>
            ))}
          </div>
        )}
      </div>
      <Footer />
    </div>
  );
};

export default MyBookings;